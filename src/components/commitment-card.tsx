...
import { ConfidenceBadge, UncertaintyBadge } from "@/components/badges";
...
export function CommitmentCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "w-full rounded-[24px] border p-4 text-left shadow-[0_12px_45px_rgba(0,0,0,0.045)] transition",
        uncertain
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        <SourceChip>{commitment.type}</SourceChip>
        <SourceChip>{commitment.source}</SourceChip>
        <ConfidenceBadge value={commitment.confidence} />
