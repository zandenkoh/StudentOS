...
export function ConflictSummaryCard({
  ...
  const summary = resolved
    ? manual
      ? conflict?.resolvedSummary ?? "StudentOS applied your manual instruction and rechecked the overlap."
      : conflict?.resolvedSummary ?? "Suggested deconflict applied."
    : conflict?.unresolvedSummary ?? "Two fixed-time items may overlap.";

  return (
    <section
      className={cn(
        "rounded-[28px] border p-5 shadow-soft",
        resolved ? "border-neutral-200 bg-neutral-50" : "border-red-200 bg-red-50"
      )}
    >
      {resolved ? (
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="flex size-11 items-center justify-center rounded-full bg-white text-neutral-700">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-2xl bg-white/75 p-3">
          <p className={cn("text-xs font-semibold", resolved ? "text-neutral-600" : "text-red-700")}>
            {conflict?.fixedEventTitle ?? "Fixed item"}
          </p>
          <p className="mt-1 font-semibold">{conflict?.fixedEventTime ?? "4:30-6:30 PM"}</p>
        </div>
        <div className="rounded-2xl bg-white/75 p-3">
          <p className={cn("text-xs font-semibold", resolved ? "text-neutral-600" : "text-red-700")}>
            {conflict?.conflictingEventTitle ?? "Other item"}
          </p>
          <p className="mt-1 font-semibold">{conflict?.conflictingEventTime ?? "5:30-6:15 PM"}</p>
        </div>
        <div className="rounded-2xl bg-white/75 p-3">
          <p className={cn("text-xs font-semibold", resolved ? "text-neutral-600" : "text-red-700")}>Overlap</p>
          <p className="mt-1 font-semibold">{resolved ? "Handled" : conflict?.overlapLabel ?? "Not confirmed"}</p>
        </div>
        <div className="rounded-2xl bg-white/75 p-3">
          <p className={cn("text-xs font-semibold", resolved ? "text-neutral-600" : "text-red-700")}>Impact</p>
